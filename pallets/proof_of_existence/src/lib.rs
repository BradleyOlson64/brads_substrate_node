...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_claim(
			...
			// Verify that the user can afford the reserve deposit
		...
		pub fn revoke_claim(
			...
			let (owner, _) = Proofs::<T>::get(&proof).expect("Already checked that value exists, safe QED");

			// Verify that sender of the current call is the claim owner.
			ensure!(sender == owner, Error::<T>::NotProofOwner);

			// Return reserved funds
			T::Token::unreserve(&sender, T::ReserveAmount::get());
